...
`use.redaction.config.redaction_term` value authored as `$${config.secret}`, then copies that
literal `${config.secret}` template into the generated interposer config. The compiled scenario
therefore retains `${config.secret}` rather than the secret value itself.
