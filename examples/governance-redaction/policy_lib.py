...
class PolicyRequest:
    ...
    @classmethod
    def from_bytes(cls, raw: bytes) -> "PolicyRequest":
        try:
            payload = json.loads(raw.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as err:
            raise RequestParseError(f"invalid JSON request body: {err}") from err
        if not isinstance(payload, dict):
            raise RequestParseError("policy request body must be a JSON object")
        raw_scope = payload.get("scope")
        if not isinstance(raw_scope, dict):
            raise RequestParseError("policy request must contain an object `scope` field")
...
def serve_policy(
    port: int,
    build_interpositions: Callable[[PolicyRequest], Iterable[InterpositionJson]],
) -> None:
    class Handler(http.server.BaseHTTPRequestHandler):
        def do_POST(self) -> None:
            try:
                ...
                request = PolicyRequest.from_bytes(raw)
