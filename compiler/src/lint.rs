...
pub(crate) use amber_manifest::lint::ManifestLint;
use amber_manifest::{
    BindingSource, ComponentDecl, ExportTarget, InterpolatedString, InterpolationSource, Manifest,
    ManifestSpans, Program, ProgramArgValue, ProgramCommandKind, ProgramConfigUseSite, SlotTarget,
    parse_slot_query, span_for_json_pointer, validate_slot_query_for_slot,
};
...
type CommandItemLocation<'a> = (
    &'a amber_manifest::ProgramArgItem,
...
#[derive(Clone, Copy)]
enum OptionalMountField {
    Path,
    Source,
}

#[derive(Clone, Copy)]
enum OptionalConfigLintSite<'a> {
    Command {
    ...
    Mount {
        index: usize,
        field: OptionalMountField,
    },
}

impl<'a> OptionalConfigLintSite<'a> {
    fn from_program_config_use_site(site: ProgramConfigUseSite<'a>) -> Option<Self> {
        match site {
            ProgramConfigUseSite::CommandValue { kind, index } => {
                Some(Self::Command { kind, index })
            }
            ProgramConfigUseSite::EnvValue { name, .. } => Some(Self::Env { name }),
            ProgramConfigUseSite::MountPath { index } => Some(Self::Mount {
                index,
                field: OptionalMountField::Path,
            }),
            ProgramConfigUseSite::MountSource { index } => Some(Self::Mount {
                index,
                field: OptionalMountField::Source,
            }),
            ProgramConfigUseSite::Image
            | ProgramConfigUseSite::Path
            ...
            | ProgramConfigUseSite::CommandWhen { .. }
            | ProgramConfigUseSite::CommandEach { .. }
            | ProgramConfigUseSite::EnvWhen { .. }
            | ProgramConfigUseSite::EnvEach { .. }
            | ProgramConfigUseSite::EndpointWhen { .. }
            | ProgramConfigUseSite::EndpointEach { .. }
            | ProgramConfigUseSite::EndpointName { .. }
            | ProgramConfigUseSite::EndpointPort { .. }
            | ProgramConfigUseSite::EndpointProtocol { .. }
            | ProgramConfigUseSite::MountWhen { .. }
            | ProgramConfigUseSite::MountEach { .. }
            | ProgramConfigUseSite::MountName { .. } => None,
        }
    }

    fn is_guarded(self, program: &Program, query: &str) -> bool {
        match self {
            Self::Command { kind, index } => command_item(program, kind, index)
                .and_then(|item| item.when())
                .is_some_and(|when| {
                    when.source() == InterpolationSource::Config && when.query() == query
                }),
            Self::Env { name } => program
                .env()
                .get(name)
                .and_then(|value| value.when())
                .is_some_and(|when| {
                    when.source() == InterpolationSource::Config && when.query() == query
                }),
            Self::Mount { index, .. } => program
                .mounts()
                .get(index)
                .and_then(|mount| mount.when.as_ref())
                .is_some_and(|when| {
                    when.source() == InterpolationSource::Config && when.query() == query
                }),
        }
    ...
    fn span(
        ...
        match self {
            Self::Command { kind, index } => {
                let Some((item, location_of, field_pointer)) =
                    command_item_with_location(program, kind, index)
                else {
                    return fallback;
                };
                let base_pointer = match &item.value {
                    ProgramArgValue::Arg(_) => format!("{field_pointer}/{index}/arg"),
                    ProgramArgValue::Argv(_) => format!("{field_pointer}/{index}/argv"),
                };
                span_for_json_pointer(source, root, &base_pointer)
                    .or_else(|| span_for_json_pointer(source, root, &location_of(index).pointer()))
                    .or_else(|| span_for_json_pointer(source, root, field_pointer))
                    .unwrap_or(fallback)
            }
            Self::Env { name } => {
                let Some(value) = program.env().get(name) else {
                    return fallback;
                };
                let pointer = if value.when().is_some() || value.each().is_some() {
                    PointerBuf::from_tokens(["program", "env", name, "value"]).to_string()
                } else {
                    PointerBuf::from_tokens(["program", "env", name]).to_string()
                };
                span_for_json_pointer(source, root, &pointer)
                    .or_else(|| {
                    ...
                    .or_else(|| span_for_json_pointer(source, root, "/program/env"))
                    .unwrap_or(fallback)
            }
            Self::Mount { index, field } => {
                let mounts_pointer = mounts_pointer(program);
                let item_pointer = format!("{mounts_pointer}/{index}");
                let field_pointer = format!(
                    "{item_pointer}/{}",
                    match field {
                        OptionalMountField::Path => "path",
                        OptionalMountField::Source => "from",
                    }
                );
                span_for_json_pointer(source, root, &field_pointer)
                    .or_else(|| span_for_json_pointer(source, root, &item_pointer))
                    .or_else(|| span_for_json_pointer(source, root, mounts_pointer))
                    .unwrap_or(fallback)
            }
        }
    }
    ...
    fn lint(
        ...
        match self {
            Self::Command { .. } => ManifestLint::OptionalCommandConfig {
                path,
                component: component.to_string(),
                src: src.clone(),
                span,
            },
            Self::Env { .. } => ManifestLint::OptionalEnvConfig {
                path,
                component: component.to_string(),
                src: src.clone(),
                span,
            },
            Self::Mount { .. } => ManifestLint::OptionalMountConfig {
                path,
                component: component.to_string(),
                src: src.clone(),
                span,
...
fn command_item(
    ...
    kind: ProgramCommandKind,
    index: usize,
) -> Option<&amber_manifest::ProgramArgItem> {
    match (program, kind) {
        (Program::Image(program), ProgramCommandKind::Entrypoint) => {
            program.entrypoint.0.get(index)
        }
        (Program::Path(program), ProgramCommandKind::Args) => program.args.0.get(index),
        _ => None,
    }
}

fn command_item_with_location(
    program: &Program,
    kind: ProgramCommandKind,
    index: usize,
) -> Option<CommandItemLocation<'_>> {
    match (program, kind) {
        (Program::Image(program), ProgramCommandKind::Entrypoint) => Some((
            program.entrypoint.0.get(index)?,
            CommandArgLintLocation::Entrypoint as fn(usize) -> CommandArgLintLocation,
            "/program/entrypoint",
        )),
        (Program::Path(program), ProgramCommandKind::Args) => Some((
            program.args.0.get(index)?,
...
fn mounts_pointer(program: &Program) -> &'static str {
    match program {
        Program::Image(_) | Program::Path(_) => "/program/mounts",
        Program::Vm(_) => "/program/vm/mounts",
        _ => "/program/mounts",
    }
}

fn program_optional_config_lints(
    manifest: &Manifest,
    component: &str,
    src: &NamedSource<Arc<str>>,
    spans: &ManifestSpans,
    ...
    let mut out = Vec::new();
    program.visit_config_uses(|site, query| {
        let Some(site) = OptionalConfigLintSite::from_program_config_use_site(site) else {
            return;
        };
        if !optional_leaf_paths.contains(query) || site.is_guarded(program, query) {
            return;
        }
        let span = site.span(program, source, root, fallback);
        out.push(site.lint(query.to_string(), component, src, span));
    });
    out
}
...
fn command_arg_required_slot_when_lints(
    ...
    for (idx, item) in items.iter().enumerate() {
        ...
        let span = span_for_json_pointer(source, root, &pointer)
            .or_else(|| span_for_json_pointer(source, root, &item_pointer))
            .or_else(|| span_for_json_pointer(source, root, field_pointer))
            .unwrap_or(fallback);
    ...
    if let Some(schema) = manifest.config_schema() {
        ...
        for leaf in schema_lints.leaves {
            ...
            let span = if leaf.pointer.is_empty() {
                ...
                span_for_json_pointer(source, schema_span, &leaf.pointer).unwrap_or(schema_span)
            };
        ...
        lints.extend(program_optional_config_lints(
            manifest,
            &component,
            &src,
            spans,
...
mod tests {
    ...
    use amber_manifest::{
        InterpolatedString, Manifest, ManifestSpans, Program, ProgramCommon, ProgramEntrypoint,
        ProgramImage, RawManifest,
    };
        ...
        let program = Program::image(
            ProgramImage::builder()
                ...
                .entrypoint(ProgramEntrypoint(vec![
                    "run".parse::<InterpolatedString>().unwrap().into(),
                ]))
    ...
    #[test]
    fn optional_mount_config_is_linted_for_unguarded_mount_fields() {
        let input = r#"
        {
          manifest_version: "0.3.0",
          config_schema: {
          ...
          program: {
            ...
            entrypoint: ["x"],
            mounts: [
              { path: "/tmp/${config.mount_path}", from: "config.mount_file" },
            ],
          },
        }
        "#;
        ...
        assert!(lints.iter().any(|lint| matches!(
            lint,
            crate::lint::ManifestLint::OptionalMountConfig { path, .. } if path == "mount_path"
        )));
        assert!(lints.iter().any(|lint| matches!(
            lint,
            crate::lint::ManifestLint::OptionalMountConfig { path, .. } if path == "mount_file"
        )));
    }

    #[test]
    fn when_suppresses_matching_optional_mount_lint_only() {
        let input = r#"
        {
          manifest_version: "0.3.0",
          config_schema: {
